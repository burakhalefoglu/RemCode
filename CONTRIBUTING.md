...
| 🔑 **Zero-Touch AI** | AI traffic and API keys never reach the relay. It is a control channel and never proxies a model call; the orchestrator holds no provider credentials at all. |
...
| 🧱 **Intent hierarchy** | Module → Spec → Feature → Code, each the oracle below. Mechanism — endpoints, tables, classes — never appears in a module ([ADR-018](docs/decisions.md#adr-018--the-primitive-unit-is-a-module-and-intent-is-a-hierarchy)). |
| ✍️ **The loop cannot write a module** | Structurally denied, proved by canary. A loop that can edit its own targets has no targets. And lowering a criterion is never one action for the human either ([ADR-019](docs/decisions.md#adr-019--three-intent-gates-and-a-module-document-the-loop-cannot-write)). |
| ⚔️ **Verifier ≠ producer** | T2's model may never be T1's. Configured the same, the gate is **void and the ladder halts** — never green ([ADR-022](docs/decisions.md#adr-022--tiered-model-assignment-and-the-verifier-is-never-the-producer)). |
| 🔕 **Silence by default** | Only a pending checkpoint may interrupt a human. Progress, streams and successful runs notify nobody — every avoidable interrupt is a defect ([ADR-015](docs/decisions.md#adr-015--the-humans-default-state-is-non-involvement)). |
| 👁️ **Decision-relevance** | Nothing is displayed that does not change a decision the user is about to make — budget is the sole exception. Equally: nothing is *hidden*; the raw trace is always reachable in full ([ADR-021](docs/decisions.md#adr-021--three-tabs-and-an-asynchronous-command-bar)). |
...
- **Self-hosted:** free and complete, always.
- **A team (M1):** paid. What it adds is **multi-person identity** — who approved which checkpoint, and a delivery record carrying more than one signature. That inherently needs shared infrastructure; it is not a feature held back from the free tier.
- **Price:** not set, deliberately. There is no product yet and no buyer conversation; a number published now would anchor the wrong value.
